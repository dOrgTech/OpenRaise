...
contract('BondingCurveControlledFactory', accounts => {
  let tx;
  let result;
  ...
  beforeEach(async function() {
    ...
    factory = await deploy.deployBondingCurveControlledFactory(project, [
  ...
  describe('Deploy StaticCurveLogic version', async () => {
    ...
    beforeEach(async function() {
      deployTx = await bondingCurveController.methods
        .deployStatic(
          factory.address,
          deployParams.owner,
    ...
    it('should emit deployed event', async () => {
      ...
      await expectEvent.inTransactionRaw(
        deployTx,
        deploy.CONTRACT_NAMES.BondingCurveFactory,
        factory.address,
        'BondingCurveDeployed'
      );
    });
    ...
    describe('Deploy', () => {
      ...
      beforeEach(async function() {
        const createdEvent = await expectEvent.inTransactionRaw(
          deployTx,
          deploy.CONTRACT_NAMES.BondingCurveFactory,
          factory.address,
          'BondingCurveDeployed'
        ...
        const deployedContracts = await getContractsFromDeployedEvent(createdEvent[0]);
...
async function getContractsFromDeployedEvent(event) {
  ...
  deployedAddresses.bondingCurve = event.returnValues.bondingCurve;
  deployedAddresses.bondedToken = event.returnValues.bondedToken;
  deployedAddresses.buyCurve = event.returnValues.buyCurve;
  deployedAddresses.sellCurve = event.returnValues.sellCurve;
  deployedAddresses.rewardsDistributor = event.returnValues.rewardsDistributor;

  contracts.bondingCurve = await ZWeb3.contract(
    await deploy.getAbi(deploy.CONTRACT_NAMES.BondingCurve),
    deployedAddresses.bondingCurve
  );
  contracts.bondedToken = await ZWeb3.contract(
    await deploy.getAbi(deploy.CONTRACT_NAMES.BondedToken),
    deployedAddresses.bondedToken
  );
  contracts.buyCurve = await ZWeb3.contract(
    await deploy.getAbi(deploy.CONTRACT_NAMES.StaticCurveLogic),
    deployedAddresses.buyCurve
  );
  contracts.sellCurve = await ZWeb3.contract(
    await deploy.getAbi(deploy.CONTRACT_NAMES.StaticCurveLogic),
    deployedAddresses.sellCurve
  );
  contracts.rewardsDistributor = await ZWeb3.contract(
    await deploy.getAbi(deploy.CONTRACT_NAMES.RewardsDistributor),
    deployedAddresses.rewardsDistributor
  );
