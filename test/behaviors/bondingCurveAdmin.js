  ...
  contract('Bonding Curve Admin', async accounts => {
    ...
    describe('Curve Admin', async () => {
      ...
      it('should allow owner to set new owner', async () => {
        ...
        const tx = await bondingCurve.transferOwnership(newOwner, {from: oldOwner});
      ...
      it('should not allow old owner to set new beneficiary after ownership transfer', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);
        ...
        const tx = await bondingCurve.transferOwnership(newOwner, {
          from: oldOwner
        });
      ...
      it('should allow owner to set new buy curve', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);

        const tx = await bondingCurve.setBuyCurve(constants.ZERO_ADDRESS, {
          from: curveOwner
        });
        expect(await bondingCurve.buyCurve({from: miscUser})).to.be.equal(constants.ZERO_ADDRESS);
      });

      it('should not allow non-owner to set new buy curve', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);
      ...
      it('should allow owner to set new reserve percentage', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);

        const newReservePercentage = '20';

        const tx = await bondingCurve.setReservePercentage(newReservePercentage, {
          from: curveOwner
        });
      ...
      it('should not allow non-owner to set new reserve percentage', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);
      ...
      it('should allow owner to set new dividend percentage', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);

        const newDividendPercentage = '20';

        const tx = await bondingCurve.setDividendPercentage(newDividendPercentage, {
          from: curveOwner
        });
      ...
      it('should not allow non-owner to set new dividend percentage', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);
