...
const bondingCurvePaymentTests = async (suiteName, config) => {
  contract('Bonding Curve Payment', async accounts => {
    ...
    describe('Payments', async () => {
      ...
      it('should not allow payments of amount 0', async () => {
      ...
      it('should register payments', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
        const {bondingCurve, paymentToken} = await eco.init(web3);
        await eco.bulkMint(
          paymentToken,
          accountsConfig.minter,
          [curveOwner, miscUser],
          userBalances
        );
      ...
      it('should not allow pay with greater amount than senders balance', async () => {
        const eco = new CurveEcosystem(accountsConfig, config);
      ...
      describe('Beneficiary / Dividend Split', async () => {
        ...
        it('should register correct split between beneficiary and dividend pool from non-curve owner', async () => {
          const eco = new CurveEcosystem(accountsConfig, config);
          const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);
          await eco.bulkMint(
            paymentToken,
        ...
        it('should register correct split between beneficiary and dividend pool from curve owner', async () => {
          const eco = new CurveEcosystem(accountsConfig, config);
          const {bondingCurve, paymentToken, bondedToken, buyCurve} = await eco.init(web3);
          await eco.bulkMint(
            paymentToken,
        ...
        // it('should transfer correct token amounts between beneficiary and dividend pool', async () => {
